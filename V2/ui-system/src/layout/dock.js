...
export function dockWindow(windowId, side, targetPanelId = null) {
    ...
    // Save floating size before docking (so we can restore it when undocking)
    winState.floatingSize = {
        width: winState.element.offsetWidth,
        height: winState.element.offsetHeight,
        x: winState.element.offsetLeft,
        y: winState.element.offsetTop
    };
...
export function undockWindow(windowId) {
    ...
    const floatLayer = document.getElementById('sl-float-layer');
    if (floatLayer) {
        // Use saved floating size if available, otherwise fall back to options or defaults
        const savedSize = winState.floatingSize;
        const width = savedSize?.width || winState.options.width || 400;
        const height = savedSize?.height || winState.options.height || 300;
        
        // Try to restore previous position, or center if not available
        let x, y;
        if (savedSize?.x !== undefined && savedSize?.y !== undefined) {
            // Ensure the window is still visible on screen
            x = Math.min(savedSize.x, window.innerWidth - 100);
            y = Math.min(savedSize.y, window.innerHeight - 100);
            x = Math.max(0, x);
            y = Math.max(0, y);
        } else {
        ...
        winState.element.style.left = `${x}px`;
        winState.element.style.top = `${y}px`;
