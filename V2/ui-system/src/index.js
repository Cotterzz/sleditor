...
function toggleUserMenu() {
    ...
    // Position menu based on toolbar position
    // For right toolbar, menu opens to the left of the button
    if (toolbarPos === 'left' || toolbarPos === 'float') {
        menu.style.left = `${rect.right + 8}px`;
        menu.style.bottom = `${window.innerHeight - rect.bottom}px`;
    } else if (toolbarPos === 'right') {
        // Position to the left of the button, will adjust after measuring
    ...
    // For right toolbar, position after measuring menu width
    if (toolbarPos === 'right') {
    ...
    // Ensure menu doesn't go off screen (general bounds check)
